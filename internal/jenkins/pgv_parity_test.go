...
// output for the same build (E3SP_SDK build 5). Synthetic wrapper IDs differ
// between sources and are excluded from the comparison.
func TestFlattenPGVTreeParityWithBlueOcean(t *testing.T) {
	pgvBytes, err := os.ReadFile("testdata/pgv_tree_e3sp_5.json")
	if err != nil {
	...
	blueBytes, err := os.ReadFile("testdata/blue_nodes_e3sp_5.json")
	if err != nil {
