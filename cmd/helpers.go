...
func resolveJobArgs(cmd *cobra.Command, args []string, needBuild bool) (*api.Client, string, int, error) {
	...
	if jobPath == "" {
		...
		case "git-remote":
			fmt.Fprintf(os.Stderr, "warning: guessed job from git remote: %s — use .jk.yml or pass job arg if incorrect\n", jobPath)
		case "dirname":
			fmt.Fprintf(os.Stderr, "warning: guessed job from directory name: %s — use .jk.yml or pass job arg if incorrect\n", jobPath)
		}
	}
