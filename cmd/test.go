...
func runTest(cmd *cobra.Command, args []string) error {
	...
	if report == nil {
		fmt.Fprintf(os.Stderr, "No test results for build #%d\n", buildNum)
		return nil
	}
	...
	if newFailures {
		...
		if prevReport == nil {
			fmt.Fprintln(os.Stderr, "warning: no test report for previous build — showing all failures")
			failedOnly = true
			newFailures = false
		} else {
	...
	if len(cases) == 0 {
		if failedOnly {
			fmt.Fprintln(os.Stderr, "No failed tests")
		} else {
