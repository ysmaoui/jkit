...
Plugin prerequisite: **PGV v803+** (URL_NAME `stages`). Verified on
`jenkins.ioi.imfa.hub.vwgroup.com` against build
`E3SP_SDK/e3_aux_elapsedtimemeter/feature/pod-dynamic-request-values #5`.
...
- **Acceptance:** Types compile; unit test decodes captured sample payload from `E3SP_SDK` build 5 into nested tree.
...
- **Acceptance:** Unit test: feed saved PGV payload, assert resulting `[]Stage` matches Blue Ocean `/nodes/` shape for leaf stages (same IDs, durations, parent chain) for the validated build.
...
E2E against `jenkins.ioi.imfa.hub.vwgroup.com/job/E3SP_SDK/job/e3_aux_elapsedtimemeter/job/feature%2Fpod-dynamic-request-values/5`:

1. `JK_PIPELINE_SOURCE=pgv jk diagnose E3SP_SDK/e3_aux_elapsedtimemeter/feature%2Fpod-dynamic-request-values 5`
2. `JK_PIPELINE_SOURCE=blueocean jk diagnose ...` — same command
3. `JK_PIPELINE_SOURCE=auto jk diagnose ...` — default path
4. `jk status ... 5 --stages` — verify nested parallel rendering
5. `jk log ... 5 --stage <nodeId>` — leaf, parallel branch, `PARALLEL_BLOCK` (should yield empty or fall back cleanly)
